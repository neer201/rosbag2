...
class MetadataWriter:
    ...
    def add_rel_file_path(self, fp: pathlib.Path):
        """Takes a pathlib.Path object, and (if it's not an absolute path), adds it to the internal relative filepaths list"""
        if fp.is_absolute():
            raise ValueError(print_error("Cannot add absolute path to relative filepaths. Got path: \"{}\"".format(fp)))
    ...
    def add_multiple_rel_file_paths(self, fps: List[pathlib.Path]):
        for p in fps:
            if p.is_absolute():
                raise ValueError(print_error("Cannot add absolute path to relative filepaths. Got path: \"{}\"".format(p)))
    ...
    def add_topic(self, topic_name: str, topic_type: str, topic_ser_fmt: str, topic_qos: str, topic_count: int):
        if topic_count < 0:
            raise ValueError(print_error("Topic message count cannot be negative. Got {}".format(topic_count)))
    ...
    def write_yaml(self, dir: pathlib.Path):
        """Writes a metadata.yaml file to the directory pointed to by dir"""
        if not dir.is_dir():
            raise ValueError(print_error("Expected a directory to output yaml file to. Got path: \"{}\"".format(dir)))
        
        # Create the dictionary for export
        yaml_dict = {"rosbag2_bagfile_information": self._as_yaml_dict()}

        fp = dir / "metadata.yaml"
        with fp.open('w') as yaml_file:
            # yaml_text = yaml.dump(yaml_dict, default_flow_style=False, width=1000, sort_keys=False).replace("-", "\t-")
            yaml_text = yaml.dump(yaml_dict, Dumper=RosbagYamlDumper, default_flow_style=False, width=1000, sort_keys=False)
